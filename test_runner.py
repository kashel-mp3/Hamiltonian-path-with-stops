...
for i in range(number_of_tests - 1):
    ...
    if outputs[0][i]:
        ...
        if(outputs[1][i] == 0 or (outputs[1][i] == -1 and outputs[0][i] != -1)):
            ...
            acc_greedy = min(1,float(1) - ((float(outputs[0][i]) - float(outputs[1][i])) / float(outputs[0][i])))
        if(outputs[2][i] == 0 or (outputs[2][i] == -1 and outputs[0][i] != -1)):
            acc_genetic = 0
        else:
            acc_genetic = min(1, float(1) - ((float(outputs[0][i]) - float(outputs[2][i])) / float(outputs[0][i])))
