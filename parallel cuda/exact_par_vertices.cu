    ...
    int *d_all_threads_paths)
{
  ...
  while (stack_top != -1)
  {
    ...
    {
      int final_path_max_segment_len = u_state.max_l;
      if (final_path_max_segment_len < thread_local_min_max_l_val)
      {
        thread_local_min_max_l_val = final_path_max_segment_len;
        for (int k = 0; k < n; ++k)
        {
    ...
    {
      int v_node = v_node_candidate_idx;

      if (d_graph[u_state.current_node_val * n + v_node] > 0 && !visited[v_node])
      {
        dfs_stack[stack_top].child_idx_to_try = v_node_candidate_idx + 1;

        current_path[u_state.path_pos] = v_node;
        visited[v_node] = true;
  ...
  d_all_threads_min_max_l[start_node_idx] = thread_local_min_max_l_val;
  if (thread_local_min_max_l_val != INT_MAX)
  {
    for (int k = 0; k < n; ++k)
    {
      d_all_threads_paths[start_node_idx * n + k] = thread_local_best_path_arr[k];
    }
  }
}
...
int main(int argc, char **argv)
{
  if (argc < 2)
  {
    std::cerr << "Usage: " << argv[0] << " <path_to_json_data_file> [num_cuda_threads_per_block (optional, default 256)]" << std::endl;
    return 1;
  }
  ...
  if (argc > 2)
  {
    try
    {
      ...
      if (threads_per_block <= 0 || threads_per_block > 1024 || (threads_per_block > 0 && (threads_per_block & (threads_per_block - 1)) != 0))
      {
        std::cerr << "Threads per block must be > 0, <= 1024, and ideally a power of 2. Using default 256." << std::endl;
        threads_per_block = 1024;
      }
    }
    catch (const std::exception &e)
    {
      std::cerr << "Invalid argument for threads per block. Using default 256. Error: " << e.what() << std::endl;
      threads_per_block = 1024;
    }
  }
  ...
  CUDA_CHECK(cudaMalloc((void **)&d_all_threads_min_max_l, n * sizeof(int)));
  CUDA_CHECK(cudaMalloc((void **)&d_all_threads_paths, n * n * sizeof(int)));
  ...
  std::vector<int> h_initial_min_max_l_for_threads(n, INT_MAX);
  CUDA_CHECK(cudaMemcpy(d_all_threads_min_max_l, h_initial_min_max_l_for_threads.data(), n * sizeof(int), cudaMemcpyHostToDevice));

  int num_total_threads_needed = n;
  int num_blocks = (num_total_threads_needed + threads_per_block - 1) / threads_per_block;

  std::cout << "Launching CUDA kernel (cooperative pruning): n=" << n
            << ", Blocks=" << num_blocks
            << ", Threads/Block=" << threads_per_block << std::endl;
  ...
  std::vector<int> h_all_threads_min_max_l(n);
  std::vector<int> h_all_threads_paths(n * n);
  CUDA_CHECK(cudaMemcpy(h_all_threads_min_max_l.data(), d_all_threads_min_max_l, n * sizeof(int), cudaMemcpyDeviceToHost));

  CUDA_CHECK(cudaMemcpy(h_all_threads_paths.data(), d_all_threads_paths, n * n * sizeof(int), cudaMemcpyDeviceToHost));

  int global_best_min_max_l = INT_MAX;
  std::vector<int> global_opt_path(n);
  int best_start_node_idx = -1;

  for (int i = 0; i < n; ++i)
  {
    if (h_all_threads_min_max_l[i] < global_best_min_max_l)
    {
      global_best_min_max_l = h_all_threads_min_max_l[i];
      best_start_node_idx = i;
    }
  }

  if (best_start_node_idx != -1)
  {
    for (int k = 0; k < n; ++k)
    {

      global_opt_path[k] = h_all_threads_paths[best_start_node_idx * n + k];
    }
  }
  ...
  if (global_best_min_max_l == INT_MAX)
  {
    std::cout << "-1" << std::endl;
  }
  else
  {
    for (int i = 0; i < n; ++i)
    {
      std::cout << global_opt_path[i] << (i == n - 1 ? "" : " ");
    }
  ...
  CUDA_CHECK(cudaFree(d_all_threads_min_max_l));
  CUDA_CHECK(cudaFree(d_all_threads_paths));
